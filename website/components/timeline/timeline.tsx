...
export const TimelineView: React.FC = () => {
	...
	const { data: calendarEvents = [] } = useAcademicCalendar(defaultYear);
								...
								>
									{filteredCalendarEvents.map((event, idx) => {
										...
										return (
											...
											>
												<div
													className={`border rounded px-2 py-1 text-xs truncate flex items-center gap-1.5 shadow-sm transition-colors cursor-default ${
														isHoliday
															? "bg-red-500/10 border-red-500/20 text-red-300 hover:bg-red-500/20"
															: "bg-[#543A14]/40 border-[#F0BB78]/30 text-[#F0BB78] hover:bg-[#543A14]/60"
													}`}
												>
													<div
